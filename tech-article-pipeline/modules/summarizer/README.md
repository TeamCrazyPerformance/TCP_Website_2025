...
| `maximumSummaryLength` | integer | 1 이상 | 상세 요약 최대 문자 수 |
| `maximumOneLineSummaryLength` | integer | 1 이상 | 한 줄 요약 최대 문자 수 |
...
Gemini 출력은 `application/json`과 호출별 JSON Schema로 제한한 뒤 Pydantic과 업무 규칙으로 다시 검증한다. 태그, 요약 길이, 한 줄 여부 및 번역 옵션의 `null` 조건을 만족하지 않으면 성공 결과로 반환하지 않는다. 텍스트 길이 또는 빈 문자열 조건만 위반한 경우에는 최대 한 번 재생성하며 입력·출력 토큰 수에는 두 호출의 사용량을 합산한다.
...
4. 제목과 본문을 JSON으로 직렬화하여 `<article_data>` 구분자 안에 넣고 Gemini를 호출한다.
5. 응답 JSON을 파싱하고 Pydantic으로 검증한다.
6. 태그, 요약 길이, 최소 글자 수, 한 줄 여부와 번역 조건을 추가 검증한다.
7. 텍스트 제약을 위반하면 최대 한 번 재호출한다. 최대 길이를 넘긴 필드의 재생성 목표는 원래 제한의 90%로 낮춘다.
