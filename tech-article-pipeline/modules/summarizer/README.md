...
| 기본 프롬프트 버전 | `dev-news-summary-v16` |
| 테스트 | pytest |
...
$env:GEMINI_PROMPT_VERSION = "dev-news-summary-v16"
...
| `maximumSummaryLength` | integer | 1 이상 | 상세 요약 문장만 합산한 글자 수의 절대 상한 |
...
코어 파이프라인이 기본으로 전달하는 상한은 상세 요약 600자, 한 줄 요약 100자다.
상세 요약은 450~550자를 권장 범위로 삼되, 짧은 기사에는 최소 분량을 강제하지
않는다. 상세 요약 글자 수는 `keyPoints`와 `checkPoints`의 문장만 합산하며 섹션
제목, 불릿 기호, 줄바꿈과 한 줄 요약은 제외한다. 생성 결과에는 별도 허용 오차를
...
작으면 해당 최대 길이를 최소 길이로 사용한다. 구조화 상세 요약은 4~7개의
`keyPoints`와 선택적인 0~2개의 `checkPoints`를 사용하며 별도 도입 문단은 만들지
않는다. 제목이나 본문 번역을 활성화한 경우 번역 결과는 공백이 아닌 문자열이어야
한다.
...
- Gemini는 Markdown 대신 문자열 배열인 `keyPoints`와 `checkPoints`를 반환하고 프로그램이 `summary` Markdown 문자열로 변환한다.
- 핵심 요약은 주체·기술명, 가장 중요한 변화와 원문에 명시된 결과·영향·제약을 한 문장으로 명확히 연결하고, 모호한 소개 표현이나 기능 나열을 피한다.
- 상세 요약에는 별도 도입 문단을 만들지 않는다. 이해에 필요한 배경, 기존 방식 또는 발표 맥락은 첫 번째 핵심 항목에 포함한다.
- 상세 요약 문장은 기본 상한 600자 안에서 450~550자를 권장한다. 기사가 짧으면 핵심 사실을 반복하거나 추측해 최소 분량을 채우지 않는다.
- 핵심 항목은 기본 5개이고 원문의 정보량에 따라 4~7개를 사용한다. 각 항목은 하나의 중심 사실을 담은 완결된 문장으로 작성하고 작동 방식, 적용 범위와 사례를 우선한다.
- 핵심 주장, 효과, 성능, 규모 또는 성숙도를 뒷받침하는 수치·전후 비교·측정 조건은 수치 없는 일반 설명보다 우선한다. 중요한 근거가 여러 개면 정보량에 맞춰 6~7개 항목을 사용한다.
- 확인할 점은 도입 판단이나 해석에 직접 영향을 주는 베타 여부, 미지원 범위, 적용·측정 환경, 검증 조건, 호환성, 운영 제약 또는 후속 확인 사항이 있을 때만 1~2개를 사용한다. 남은 사실을 옮기는 보조 목록으로 사용하지 않는다.
- 기업, 연구진 또는 작성자가 제시한 수치와 결과는 주체를 드러내고, 해당 주체의 평가와 기사에서 확인되는 사실을 구분한다.
- 지원·미지원, 베타·GA, 기본 제공·별도 설정, 네이티브 실행·변환 실행처럼 의미가 달라지는 상태를 보존하며 기술명, 기호, 버전과 대소문자를 임의로 바꾸지 않는다. `q*`처럼 기호가 의미의 일부인 표기도 보존한다.
- 한국어 제목은 간결한 뉴스 헤드라인 형태로 만들고, 한 줄 요약은 `-합니다`, `-됩니다`, `-있습니다` 형태의 존댓말 서술체를 유지한다. `keyPoints`와 `checkPoints`는 `-한다`, `-된다`, `-있다`, `-했다` 형태의 간결한 `-다` 문어체로 작성하며 `-함`, `-됨` 메모체는 사용하지 않는다.
- 한국어 결과는 프로그램에서도 문장 수와 `-니다.` 종결을 검사하며, 제목의 `-됨` 또는 문장형 `-니다` 종결도 재생성 대상으로 처리한다.
- 한국어 결과에 원문에 없던 일본어 가나가 섞이면 혼합 문자 오류로 판단해 한 번 재생성한다. 원문 고유명사에 포함된 가나는 허용한다.
- 한 줄 요약과 목록 사이에서 같은 사실을 반복하지 않고 인라인 레이블, 굵은 글씨, 표, 링크, 인용문 및 Markdown 문법을 모델 출력에 포함하지 않는다.
- 확인할 근거가 충분하지 않으면 `checkPoints`를 빈 배열로 반환하며 프로그램은 `확인할 점` 섹션을 생략한다.
...
```json
{
  ...
  "enrichment": {
    ...
    "summary": "### 주요 내용\n\n- 새 API가 해결하려는 기존 개발 방식의 문제와 도입 배경을 설명한다.\n- 주요 구성 요소가 요청을 처리하고 결과를 반환하는 흐름을 정리한다.\n- 기존 구현과 비교해 달라진 동작을 구체적으로 설명한다.\n- API가 적용되는 범위와 함께 제공되는 기능을 정리한다.\n\n### 확인할 점\n\n- 실제 도입 전에 원문에 명시된 호환성 조건과 운영 제약을 확인해야 한다.",
