...
| `maximumSummaryLength` | integer | 1 이상 | 상세 요약 생성 목표 문자 수 |
| `maximumOneLineSummaryLength` | integer | 1 이상 | 한 줄 요약 생성 목표 문자 수 |
...
코어 파이프라인의 운영 기본 생성 목표는 상세 요약 800자, 한 줄 요약
100자다. Gemini의 작은 길이 오차로 불필요하게 재호출하지 않도록 결과는
상세 요약은 생성 목표보다 50자, 한 줄 요약은 생성 목표보다 10자까지
추가로 허용한다. 호출자가 다른 값을 전달해도 같은 필드별 허용치를 적용한다.
...
7. 텍스트 제약을 위반하면 최대 한 번 재호출한다. 상세 요약은 생성 목표보다 50자, 한 줄 요약은 10자를 초과해 허용 상한을 넘긴 경우에만 길이 재생성을 수행하며, 해당 필드의 재생성 목표는 원래 목표의 90%로 낮춘다.
