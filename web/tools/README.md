...
## mock-tech-articles-api.mjs — 웹사이트 프론트엔드용 목 서버

```bash
PORT=4000 node tools/mock-tech-articles-api.mjs   # 단독 실행
```

홈·공지·스터디·멤버·팀·모집 공개 화면과 기술 아티클 관리자 목록·검수 큐·공개 화면,
크롤링 관리 화면이 쓰는 응답을 흉내 냅니다. 크롤링 실행
...
기본 실행에서는 운영 공개 화면과 같은 규모인 공개 아티클 106건과 새 글 9건을 보여준다.
첫 페이지 20건은 운영 화면의 제목·한 줄 요약·출처·태그 조합을 기준으로 구성했고, 나머지
24건은 관리자 검수 흐름을 확인하기 위한 비공개 상태 데이터다. 규모를 바꿔야 할 때는
`MOCK_ARTICLE_COUNT`와 `MOCK_PUBLIC_ARTICLE_COUNT` 환경 변수를 사용할 수 있다.
...
로컬 데모 로그인 후 `/study/1`(React & TypeScript 실전 스터디)에서 가상 회원 8명을
확인할 수 있습니다. 스터디장·스터디장 후보·일반 회원과 다음 표시 사례를 포함합니다.

- 다양한 전공이 입력된 회원
- 긴 전공명이 있는 회원
- 전공 미입력 회원

`/study/2`에는 4명, `/study/3`과 `/study/4`에는 각각 3명이 있습니다.
스터디 응답의 `members`는 실제 API와 동일하게 `major`, `profile_image`를
회원 객체 바로 아래에 반환합니다. 자기소개는 포함하지 않습니다. 이 데이터는 스터디 화면 확인용이며,
공개 멤버 목록의 목업과는 별도로 관리합니다.

`dev.sh`로 실행 중이면 목업 파일 저장 시 자동 재시작되므로 브라우저를 새로고침하면 됩니다.
직접 실행한 목 서버는 재시작해야 변경된 데이터를 읽습니다.
