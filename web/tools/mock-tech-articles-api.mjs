...
// Pull a few published articles' collection times forward so the NEW badge shows
// up locally. The fixed base time would otherwise put everything past 12 hours.
articles
  .filter(
    (a) =>
...
// Same predicates as tech_article_pipeline.persistence.mysql.STAGE_PREDICATES.
// Change one side only and the mock renders differently from production.
...
// Seed a few so the counts render. One article gets more guest views than member
// views, which is why the two numbers are shown separately.
...
const demoStudies = [
  ...
  {
    id: 5,
    study_name: "운영진 내부 세미나 (비공개)",
    start_year: 2026,
    study_description:
      "비공개 스터디입니다. 로그인하지 않은 요청에는 목록에 나오지 않아야 합니다.",
    tag: "운영,내부",
    recruit_count: 5,
    period: "2026.09 ~ 2026.12",
    apply_deadline: "2026-09-30T23:59:59+09:00",
    place: "동아리방",
    way: "격주 금요일",
    cycle: "격주",
    is_public: false,
    leader: { user_id: "demo-leader-1", name: "김민준" },
    members: [{ user_id: "demo-leader-1", name: "김민준", role: "LEADER" }],
  },
  {
    id: 4,
  ...
  if (method === "GET" && pathname.startsWith(`${ADMIN_BASE}/`)) {
    ...
    return [
      200,
      {
        ...
        evaluation: evaluationOf(found), // The admin detail nests its dimensions
      },
    ];
  }
