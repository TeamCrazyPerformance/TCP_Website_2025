...
const SOURCES = [
  { id: "cloudflare-blog", name: "Cloudflare Blog", type: "RSS", domain: "blog.cloudflare.com", path: "/rss" },
  { id: "infoq", name: "InfoQ", type: "RSS", domain: "www.infoq.com", path: "/rss/news" },
  { id: "sdtimes", name: "SD Times", type: "RSS", domain: "sdtimes.com", path: "/feed" },
];
...
const TITLES = [
  ["엣지 런타임의 콜드 스타트를 40% 줄인 방법", "How we cut edge cold starts by 40%"],
  ["대규모 Kafka 클러스터 운영에서 배운 것", "Lessons from running Kafka at scale"],
  ["쿠버네티스 1.34 스케줄러 변경점 정리", "What changed in the Kubernetes 1.34 scheduler"],
  ["Rust 비동기 클로저 안정화가 바꾸는 것", "Stabilized async closures in Rust"],
  ["LLM 추론 비용을 절반으로 줄인 캐싱 전략", "Halving LLM inference cost with caching"],
  ["PostgreSQL 18의 논리적 복제 개선", "Logical replication improvements in PostgreSQL 18"],
  ["제로 트러스트 네트워크 도입 6개월 회고", "Six months of zero trust networking"],
  ["모노레포에서 빌드 시간을 지키는 방법", "Keeping build times sane in a monorepo"],
  ["React 서버 컴포넌트 실전 도입기", "React Server Components in production"],
  ["관측 가능성 비용을 통제하는 샘플링 설계", "Designing sampling to control observability cost"],
  ["WASM으로 플러그인 시스템을 다시 만들기", "Rebuilding our plugin system on WASM"],
  ["점진적 타입 마이그레이션 3년의 기록", "Three years of gradual type migration"],
  ["장애 대응 훈련을 습관으로 만드는 법", "Making incident drills a habit"],
  ["모바일 앱 시작 시간 예산 관리하기", "Budgeting mobile app startup time"],
  ["오픈소스 유지보수자의 번아웃 다루기", "Handling maintainer burnout in open source"],
  ["gRPC에서 HTTP/3로 옮기며 배운 것", "What we learned moving gRPC to HTTP/3"],
  ["벡터 데이터베이스 선택 기준 정리", "Criteria for choosing a vector database"],
  ["CI 캐시 적중률을 90%로 끌어올리기", "Pushing CI cache hit rate to 90%"],
  ["설계 문서를 실제로 읽게 만드는 방법", "Getting design docs actually read"],
  ["공급망 보안을 위한 SBOM 실전 적용", "Applying SBOM for supply chain security"],
];
...
const iso = (daysAgo, hourOffset = 0) =>
  new Date(Date.UTC(2026, 7, 20, 3, 0, 0) - daysAgo * 86400000 + hourOffset * 3600000).toISOString();

const articles = Array.from({ length: ARTICLE_COUNT }, (_, index) => {
  const [title, originalTitle] = TITLES[index % TITLES.length];
  const suffix = index >= TITLES.length ? ` (${Math.floor(index / TITLES.length) + 1}편)` : "";
  ...
  const [processingStatus, reviewStatus, publicationStatus] =
    index < 6 ? REACHABLE_STATES[0] : REACHABLE_STATES[index % REACHABLE_STATES.length];
  ...
  return {
    ...
    title: title + suffix,
    originalTitle,
    authors: pickN(["김도현", "박지훈", "이서연", "Sarah Kim", "Marco Rossi"], intBetween(1, 2)),
...
for (const seed of articles.filter((a) => a.processingStatus === "PROCESSING_FAILED").slice(0, 1)) {
...
function articleStage(a) {
  switch (a.processingStatus) {
    ...
    case "QUALITY_EVALUATED": return "QUALITY_REVIEW";
    case "ENRICHMENT_PENDING": return "ENRICHING";
    case "QUALITY_REJECTED": return "QUALITY_REJECTED";
    case "ENRICHED":
      return a.reviewStatus === "PENDING" && a.publicationStatus === "UNPUBLISHED"
        ? "PUBLICATION_REVIEW"
        : "COMPLETED";
    case "PROCESSING_FAILED":
      return resolvedApprovals.has(a.articleId) ? "FAILED_AFTER_APPROVAL" : "FAILED";
    default: return "UNKNOWN";
  }
}

const STAGE_NAMES = [
  "INGESTED", "QUALITY_REVIEW", "ENRICHING", "PUBLICATION_REVIEW",
  "COMPLETED", "FAILED_AFTER_APPROVAL", "FAILED", "QUALITY_REJECTED",
];

// 검토 상태 표시 오류. 단계 축과 별개입니다.
const APPROVED_COMPATIBLE = ["ENRICHMENT_PENDING", "ENRICHED", "PROCESSING_FAILED"];
const hasStatusMismatch = (a) =>
  a.reviewStatus === "APPROVED" && !APPROVED_COMPATIBLE.includes(a.processingStatus);
...
const isPublic = (a) => a.processingStatus === "ENRICHED" && a.publicationStatus === "PUBLISHED";
const byNewest = (x, y) => new Date(y.originalPublishedAt) - new Date(x.originalPublishedAt);
    ...
    candidates: [
      {
        ...
        article: { articleId: matched.articleId, title: matched.title, source: matched.source,
                   articleUrl: matched.canonicalUrl, originalLanguage: matched.originalLanguage,
                   originalPublishedAt: matched.originalPublishedAt },
      },
    ],
    matched: i === 4 ? null : { articleId: matched.articleId, title: matched.title, source: matched.source,
               articleUrl: matched.canonicalUrl, originalLanguage: matched.originalLanguage,
               originalPublishedAt: matched.originalPublishedAt },
...
const publicationQueue = () =>
  articles
    ...
    .filter((a) => a.processingStatus === "ENRICHED"
      && a.reviewStatus === "PENDING"
      && a.publicationStatus !== "PUBLISHED")
    .map((a) => ({ ...adminItem(a), reason: "공개 검토 승인 대기 중입니다.", queuedAt: a.normalizedAt }));

let publicationPolicy = { policy: "REVIEW", recordVersion: 1, updatedAt: iso(3), updatedBy: "mock-admin" };

const crawlRuns = new Map();

const CRAWL_SOURCES = {
  items: [
    { sourceId: "cloudflare-blog", name: "Cloudflare Blog", domain: "blog.cloudflare.com",
      capabilities: [{ sourceType: "RSS", sectionKey: "BLOG" }],
      crawlOptions: {
        maximumArticleCount: { default: 10, minimum: 1, maximum: 100 },
        maximumAgeHours: { default: 720, minimum: 1, maximum: 8760 },
        maximumPageCount: { default: 1, minimum: 1, maximum: 10 },
        requestTimeoutMs: { default: 15000, minimum: 1000, maximum: 60000 },
      } },
    { sourceId: "infoq", name: "InfoQ", domain: "www.infoq.com",
      capabilities: [
      ...
      crawlOptions: {
        maximumArticleCount: { default: 20, minimum: 1, maximum: 100 },
        maximumAgeHours: { default: 168, minimum: 1, maximum: 8760 },
        maximumPageCount: { default: 3, minimum: 1, maximum: 10 },
        requestTimeoutMs: { default: 20000, minimum: 1000, maximum: 60000 },
      } },
    { sourceId: "sdtimes", name: "SD Times", domain: "sdtimes.com",
      capabilities: [
        { sourceType: "RSS", sectionKey: "NEWS" },
        { sourceType: "WEB_CRAWL", sectionKey: "NEWS" },
      ...
      crawlOptions: {
        maximumArticleCount: { default: 15, minimum: 1, maximum: 100 },
        maximumAgeHours: { default: 336, minimum: 1, maximum: 8760 },
        maximumPageCount: { default: 1, minimum: 1, maximum: 10 },
        requestTimeoutMs: { default: 15000, minimum: 1000, maximum: 60000 },
      } },
  ],
...
const bulkResult = (items, idKey) => ({
  results: items.map((item) => ({ id: item[idKey], status: "SUCCEEDED", data: {} })),
...
function handle(method, pathname, query, body) {
  ...
  if (method === "GET" && pathname === "/api/v1/main/statistics") {
    return [200, { totalMembers: 128, projects: 42, awards: 17, employmentRate: 86 }];
  }
  ...
  const matches = (a) =>
    !keyword || a.title.includes(keyword) || (a.oneLineSummary || "").includes(keyword);
  ...
  if (method === "GET" && pathname === PUBLIC_BASE) {
    ...
    const rows = articles
      .filter(isPublic)
      .filter(matches)
      .filter((a) => !selected.length || a.tags.some((t) => selected.includes(t)))
      .sort(byNewest)
      .map(publicItem);
    return [200, { ...paginate(rows, page, pageSize), lastCrawledAt: LAST_CRAWLED_AT }];
  }
  ...
  if (method === "GET" && pathname.startsWith(`${PUBLIC_BASE}/`)) {
    ...
    if (!found) return [404, { statusCode: 404, message: "공개되지 않았거나 찾을 수 없는 아티클입니다." }];
    return [200, {
      ...publicItem(found),
  ...
  if (method === "GET" && pathname === `${ADMIN_BASE}/reviews/quality`) {
    return [200, paginate(qualityCases.filter((c) => !keyword || c.title.includes(keyword)), page, pageSize)];
  }
  if (method === "GET" && pathname === `${ADMIN_BASE}/reviews/publication`) {
    return [200, paginate(publicationQueue().filter((c) => !keyword || c.title.includes(keyword)), page, pageSize)];
  }
  ...
  if (method === "POST" && pathname === `${ADMIN_BASE}/crawl-runs`) {
    ...
    const run = {
      ...
      items: [],
      statistics: { fetched: 0, normalized: 0, duplicated: 0, ingested: 0 },
      error: null,
      _polls: 0,
    };
    crawlRuns.set(crawlRunId, run);
    return [202, { ...run, _polls: undefined }];
  }
  if (method === "GET" && pathname.startsWith(`${ADMIN_BASE}/crawl-runs/`)) {
    const id = decodeURIComponent(pathname.slice(`${ADMIN_BASE}/crawl-runs/`.length));
    const run = crawlRuns.get(id);
    if (!run) return [404, { statusCode: 404, message: "수집 실행을 찾을 수 없습니다." }];
    // 폴링할 때마다 QUEUED -> RUNNING -> COMPLETED 로 진행하는 척한다
    run._polls += 1;
    if (run._polls === 1) {
      Object.assign(run, { status: "RUNNING", jobStatus: "IN_PROGRESS",
        job: { status: "IN_PROGRESS", attemptCount: 1, maxAttempts: 3 },
        statistics: { fetched: 6, normalized: 4, duplicated: 1, ingested: 3 } });
    } else if (run._polls >= 2) {
      Object.assign(run, { status: "COMPLETED", jobStatus: "SUCCEEDED",
        job: { status: "SUCCEEDED", attemptCount: 1, maxAttempts: 3 },
        statistics: { fetched: 12, normalized: 11, duplicated: 2, ingested: 9 },
        items: Array.from({ length: 9 }, (_, i) => ({
          crawlItemId: `item-mock-${i + 1}`, title: `${TITLES[i % TITLES.length][0]}`, status: "INGESTED",
        })) });
    }
    return [200, { ...run, _polls: undefined }];
  }

  /* ---------- 관리자: 게시 상태 변경 ---------- */
  if (method === "POST" && pathname === `${ADMIN_BASE}/publication-actions/bulk`) {
    const items = body?.items || [];
    items.forEach(({ articleId, action }) => applyPublication(articleId, action));
  ...
  if (method === "POST" && pathname.endsWith("/publication-actions")) {
    const articleId = decodeURIComponent(
      pathname.slice(ADMIN_BASE.length + 1, pathname.length - "/publication-actions".length),
    );
    const updated = applyPublication(articleId, body?.action);
    if (!updated) return [404, { statusCode: 404, message: "아티클을 찾을 수 없습니다." }];
    return [200, updated];
  }

  /* ---------- 관리자: 검수 판정 ---------- */
  if (method === "POST" && pathname === `${ADMIN_BASE}/reviews/duplicates/resolutions/bulk`) {
    const items = body?.items || [];
    items.forEach(({ caseId }) => removeCase(duplicateCases, "reviewCaseId", caseId));
    return [200, bulkResult(items, "caseId")];
  }
  if (method === "POST" && pathname === `${ADMIN_BASE}/reviews/quality/resolutions/bulk`) {
    const items = body?.items || [];
    items.forEach(({ caseId }) => removeCase(qualityCases, "caseId", caseId));
    return [200, bulkResult(items, "caseId")];
  }
  if (method === "POST" && /\/reviews\/duplicates\/[^/]+\/resolutions$/.test(pathname)) {
    const caseId = decodeURIComponent(pathname.split("/").slice(-2)[0]);
    removeCase(duplicateCases, "reviewCaseId", caseId);
    return [200, { outcome: body?.action === "CONFIRM_DUPLICATE" ? "DUPLICATE" : "UNIQUE", resolution: {}, article: {} }];
  }
  if (method === "POST" && /\/reviews\/quality\/[^/]+\/resolutions$/.test(pathname)) {
    const caseId = decodeURIComponent(pathname.split("/").slice(-2)[0]);
    const target = qualityCases.find((c) => c.caseId === caseId);
    if (target) {
      const article = articles.find((a) => a.articleId === target.articleId);
      if (article) {
        article.processingStatus = body?.action === "APPROVE" ? "ENRICHMENT_PENDING" : "QUALITY_REJECTED";
        article.reviewStatus = body?.action === "APPROVE" ? "APPROVED" : "REJECTED";
        if (body?.action === "APPROVE") resolvedApprovals.add(article.articleId);
  ...
  if (method === "GET" && pathname === ADMIN_BASE) {
    ...
    if (sort === "OLDEST")
      rows = [...rows].sort((x, y) => new Date(x.updatedAt) - new Date(y.updatedAt));
    else if (sort === "SCORE_DESC") rows = [...rows].sort((x, y) => y.valueScore - x.valueScore);
    else if (sort === "SCORE_ASC") rows = [...rows].sort((x, y) => x.valueScore - y.valueScore);
  ...
  if (method === "GET" && pathname.startsWith(`${ADMIN_BASE}/`)) {
    ...
    if (!found) return [404, { statusCode: 404, message: "아티클을 찾을 수 없습니다." }];
    return [200, {
      ...adminItem(found),
...
function applyPublication(articleId, action) {
  ...
  const next = { PUBLISH: "PUBLISHED", HIDE: "HIDDEN", ARCHIVE: "ARCHIVED" }[action];
  if (next) {
    article.publicationStatus = next;
    article.publishedAt = next === "PUBLISHED" ? new Date().toISOString() : article.publishedAt;
  ...
  return { articleId, publicationStatus: article.publicationStatus,
           reviewStatus: article.reviewStatus, recordVersion: article.recordVersion };
}
...
const server = createServer((req, res) => {
  ...
  req.on("end", () => {
    ...
    try {
      [status, payload] = handle(req.method, url.pathname, url.searchParams, body);
    } catch (error) {
      status = 500;
    ...
    console.log(`${label} ${status} ${req.method} ${url.pathname}${url.search}`);
    if (status === 404 && !url.pathname.includes("tech-articles")) {
      console.log(`   ↑ 기술 아티클 화면과 무관한 경로라 목을 만들지 않았습니다. 무시해도 됩니다.`);
    }
