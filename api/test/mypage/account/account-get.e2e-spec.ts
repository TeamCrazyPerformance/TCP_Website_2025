...
describe('GET /api/v1/mypage/account (e2e)', () => {
  ...
  it('정상 인증 시 200 반환 및 개인정보 조회', async () => {
    const response = await request(app.getHttpServer())
      .get('/api/v1/mypage/account')
      .set('Authorization', `Bearer ${userToken}`);

    expect(response.status).toBe(200);
    expect(response.body).toHaveProperty('name', '테스트유저');
    expect(response.body).toHaveProperty('email', 'test@example.com');
    expect(response.body).toHaveProperty('phoneNumber', '010-1234-5678');
