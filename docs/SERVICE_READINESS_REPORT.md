...
## 1. Executive Summary
본 프로젝트는 기술 아티클 프론트엔드·API·파이프라인을 포함해 **배포 가능한 상태**이며,
실서버 오픈 전에는 아래 서버 수준 승인 항목을 확인해야 합니다.
인프라(Docker, Nginx) 구조는 매우 견고하게 설계되어 있으나, 애플리케이션 레벨(Backend)에서의 입력 검증과 보안 설정이 보완되어야 합니다.

- **종합 판정**: 🟡 **조건부 준비 완료** - 코드/자동화 완료, 실서버 Docker 복구 훈련과 SSL 확인 필요
...
### 인프라 & 배포
- **Docker Compose**: Frontend, Backend, DB, ELK 스택이 완벽하게 분리되어 관리되고 있습니다.
...
### 프론트엔드
- **API 호출**: 소스 코드에서 `fetch('/api/v1/...')`와 같이 상대 경로를 사용하고 있어, 배포 환경(도메인)이 바뀌어도 문제없이 작동합니다. (`api/client.js` 및 `About.jsx` 확인 완료)
...
## 3. ⚠️ 보완 필요 항목 (Requires Action)

### 백엔드 보안 (Backend Security)
1. **Global Validation (입력 검증)**
...
2. **Secret Key 교체**: 배포 전 `DEPLOYMENT_CHECKLIST.md`에 명시된 대로 JWT Secret, DB Password 등을 반드시 무작위 문자열로 교체해야 합니다.
...
## 2026 기술 아티클 릴리스 준비 상태

- [x] 공개·회원·관리자 프론트엔드와 API 계약 연동
- [x] pipeline MySQL migration/readiness 및 API 연동 배포 순서 고정
- [x] PostgreSQL/MySQL/파일 동일 세트 백업, SHA-256 검사, 통합 복구
- [x] 내부 시크릿 자동 생성과 Gemini/크롤러 외부 입력 분리
- [x] 공개 태그 API와 `/tech-articles`를 포함한 전체 health
- [ ] 실제 Linux 서버에서 fresh/idempotent setup과 통합 restore drill 수행
- [ ] 저장소 밖에서 주입한 `origin.crt`/`origin.key` 유효기간과 권한 확인

일반 health는 Gemini 호출을 발생시키지 않는다. 비용이 드는 smoke test는 별도 운영 승인
하에 수행한다.
