...
- 파이프라인이 저장한 `score.axes`에서 공개 표시명·기여도만 추려
  `valueScore.breakdown`으로 제공한다. 프런트에는 축 식별자·원점수·가중치·평가 정책과
  특정 계산식이 없다. axes 저장 전 자료는 서버가 당시 3축·4축 형태를 구분해 복구한다.
- 공개 목록·비회원 상세·회원 상세는 각각 정확한 allowlist projector를 사용한다. 원문,
  내부 상태·버전, 소스 ID·수집 방식, 작성자, 평가 판정·근거·신호는 브라우저 응답에 없다.
