...
## TechArticlesModule 경계

NestJS는 공개 목록/태그, JWT 상세, ADMIN 관리 API의 인증과 DTO 계약을 담당한다.
실제 아티클 상태·검수·수집 데이터는 서비스 토큰으로 FastAPI `/internal/v1`에 요청한다.
파이프라인의 MySQL/Gemini 자격 증명과 원문 payload는 NestJS 환경, 로그, 외부 오류에
노출하지 않는다. 파이프라인 장애는 기존 API 기동을 막지 않고 해당 도메인에서 안정된
503 오류로 변환된다.
