...
> 가장 일반적인 명령어: `bash CICDtools/update_all.sh`
...
| `update_all.sh` | 프론트 staging → pipeline/MySQL → API/PostgreSQL → 프론트 활성화 → health | 배포 환경에 따름 |
...
| 스크립트 | 목적 |
|----------|---------|
| `backup_db.sh` | PostgreSQL + pipeline MySQL + 파일을 SHA-256 동일 세트로 백업 |
| `restore_db.sh` | 지정한 단일 검증 세트 복구 후 양쪽 migration/health |
...
| 시나리오 | 권장 스크립트 |
|----------|---------------|
| 일반적인 코드 업데이트 | `bash CICDtools/update_all.sh` |
| 기술 아티클 설정 변경 | `update_tech_article_config.sh`에 변경 항목 하나 지정 |
| DB 비밀번호 교체 | `bash CICDtools/rotate_db_password.sh postgres|pipeline` |
...
운영과 개발 quick setup, `update_all.sh`, `migrate_db.sh`, `check_health.sh`는 모두
`tech-articles` profile을 활성화한다. 일상 배포는 `bash CICDtools/update_all.sh`,
파이프라인만 배포할 때는 `bash CICDtools/update_pipeline.sh`를 사용한다. Gemini 키,
모델, 크롤러 식별값, 서비스 토큰은 `update_tech_article_config.sh`로 한 항목씩 변경한다.
