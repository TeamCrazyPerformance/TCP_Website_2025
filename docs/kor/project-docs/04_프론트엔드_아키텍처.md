...
v9 기준 공개 목록/회원 상세와 관리자 인벤토리·중복/품질/게시 검수·수집 실행 화면이
API와 연결돼 있다. 공개 목록은 서버 pagination과 15개 표준 태그를 사용하고 상세는
JWT와 `summaryMarkdown`을 사용한다. 관리자 변경 요청은 version을 전송하며 bulk의
항목별 부분 실패를 처리한다. 빌드는 TypeScript 4.9.5와 `cross-env`를 잠가 Windows와
Linux 모두 `npm run build`가 `web/dist/`를 생성한다.
