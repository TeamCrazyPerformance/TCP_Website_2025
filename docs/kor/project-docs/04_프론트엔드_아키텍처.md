...
v9 기준 공개 목록/상세와 관리자 인벤토리·중복/품질/게시 검수·수집 실행 화면이 API와
연결돼 있다. 공개 목록과 상세 요약·출처는 로그인 없이 볼 수 있고 가치 점수만 회원
응답에 포함된다. 품질 평가 축과 가중치는 프런트에 고정하지 않고 평가 결과의
`score.axes`를 렌더링한다. 관리자 변경 요청은 version을 전송하며 bulk의 항목별 부분
실패를 처리한다. 빌드는 TypeScript 4.9.5와 `cross-env`를 잠가 Windows와 Linux 모두
`npm run build`가 `web/dist/`를 생성한다.
